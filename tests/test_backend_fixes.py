...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'text-to-cad', 'backend'))
...
def test_no_old_user_manager_methods():
    ...
    old_methods = ['check_user_can_generate', 'record_user_prompt', 'increment_user_model_count']
    
    for method in old_methods:
        assert not hasattr(user_manager, method), f"Old method {method} still exists!"
    
    # These new methods SHOULD exist
    new_methods = ['create_or_get_session', 'get_user_info', 'check_user_limit', 
                   'increment_model_count', 'track_generation']
...
async def test_generation_route_uses_async_methods():
    ...
    with patch('services.user_management.user_manager') as mock_user_manager:
        ...
        mock_user_manager.increment_model_count = AsyncMock(return_value={'model_count': 1})
...
def test_user_manager_configuration():
    ...
    print(f"✅ User manager configured with max models: {user_manager.max_models_per_user}")
...
async def test_check_user_limit_async():
    ...
    with patch.object(user_manager, 'get_user_info', new_callable=AsyncMock) as mock_get_info:

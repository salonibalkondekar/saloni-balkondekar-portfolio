...
def main():
    ...
    try:
        ...
        assert actual_components["database"] == "analytics_db", (
            f"Database should be 'analytics_db', got '{actual_components['database']}'"
        )
    ...
    try:
        ...
        health_checks = re.findall(
            r"healthcheck:.*?test:.*?\[.*?\]", compose_content, re.DOTALL
        )
